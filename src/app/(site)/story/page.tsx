...
const DEFAULT_COMMUNITY_ITEMS: ContentItem[] = [
  {
    title: "Ngư Dân",
    description:
...
export default async function StoryPage() {
  const cms = await fetchPageContent("story");
  const origin = getSection(cms, "origin");
  const heritage = getSection(cms, "heritage");
  const community = getSection(cms, "community");
  const philosophy = getSection(cms, "philosophy");
  const communityItems = community?.items?.length
    ? community.items
    : DEFAULT_COMMUNITY_ITEMS;

  return (
    <>
      <HeroSection
        subtitle={cms?.heroSubtitle || "Our Story"}
        title={cms?.heroTitle || "Câu Chuyện"}
      ...
      >
        ...
        >
          <ScrollReveal>
            <SectionHeading
              subtitle={origin?.subtitle || "Khởi Nguồn"}
              title={origin?.title || "Nơi Tất Cả Bắt Đầu"}
            />
            {(origin?.body
              ? origin.body.split("\n\n")
              : [
                  "La Mer Collection ra đời từ một tình yêu sâu sắc dành cho Vĩnh Hy \u2014 vịnh biển nhỏ bé nằm giữa núi rừng và đại dương, nơi cuộc sống trôi chậm theo nhịp thở của sóng.",
                  "Chúng tôi không xây dựng một khu resort biệt lập. Thay vào đó, chúng tôi tìm kiếm, trùng tu và tái sinh những ngôi nhà có lịch sử \u2014 biến chúng thành không gian nghỉ dưỡng sang trọng mà vẫn giữ nguyên linh hồn của nơi chốn.",
                ]
            ).map((para, i) => (
              <p
                key={i}
          ...
          <ScrollReveal delay={0.2}>
            ...
            >
              {origin?.image?.asset?.url && (
                <img
                  src={origin.image.asset.url}
                  alt=""
      ...
      >
        ...
        >
          <ScrollReveal>
            <SectionHeading
              subtitle={heritage?.subtitle || "Di Sản"}
              title={heritage?.title || "Bảo Tồn Kiến Trúc Bản Địa"}
              center
              light
            ...
            >
              {heritage?.body ||
                "Mỗi căn villa trong bộ sưu tập đều mang dấu ấn thời gian. Những ngôi nhà cổ của cư dân địa phương được trùng tu tỉ mỉ, giữ nguyên kết cấu gỗ, mái ngói và khoảng sân vườn \u2014 đồng thời nâng cấp tiện nghi đạt chuẩn quốc tế."}
            </p>
          </ScrollReveal>
        </div>
      </section>
      ...
      >
        ...
        >
          <SectionHeading
            subtitle={community?.subtitle || "Cộng Đồng"}
            title={community?.title || "Sống Cùng Vĩnh Hy"}
            center
          />
        </ScrollReveal>
        <div
      ...
      >
        <ScrollReveal>
          ...
          >
            {philosophy?.body
              ? `\u201C${philosophy.body}\u201D`
              : "\u201CChúng tôi tin rằng du lịch bền vững không phải là cách ly du khách khỏi cộng đồng, mà là kết nối họ \u2014 để mỗi chuyến đi đều để lại giá trị cho cả hai phía.\u201D"}
          </blockquote>
          <p
            className="uppercase"
            style={{
